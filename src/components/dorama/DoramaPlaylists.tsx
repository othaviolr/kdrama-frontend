  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 pointer-events-none">
        <div
          className="bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl w-full h-[90vh] sm:h-auto sm:max-h-[85vh] sm:max-w-3xl overflow-hidden pointer-events-auto animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-8 duration-300"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Header com drag indicator no mobile */}
          <div className="bg-white p-4 sm:p-6 border-b border-gray-200 sticky top-0">
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0 mr-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-gray-700 text-base sm:text-lg font-medium truncate">
                    {doramaTitulo}
                  </p>
                </div>
              </div>
          ...
          {/* Conteúdo */}
          <div className="overflow-y-auto h-full sm:max-h-[calc(85vh-140px)] p-4 sm:p-6 bg-gray-50/50">
            {loading ? (
              <div className="space-y-3 sm:space-y-4">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="flex gap-3 sm:gap-4 p-3 sm:p-4 bg-white rounded-xl sm:rounded-2xl animate-pulse border border-gray-200"
                  >
                    <div className="w-14 h-14 sm:w-20 sm:h-20 md:w-24 md:h-24 bg-gray-200 rounded-lg sm:rounded-xl flex-shrink-0" />
                    <div className="flex-1 min-w-0 space-y-2 sm:space-y-3">
                      <div className="h-4 sm:h-5 bg-gray-200 rounded w-3/4" />
                      <div className="h-3 sm:h-4 bg-gray-200 rounded w-1/2" />
                      <div className="h-3 sm:h-4 bg-gray-200 rounded w-1/3" />
                    </div>
                    <div className="w-16 h-8 sm:w-24 sm:h-10 bg-gray-200 rounded-lg flex-shrink-0" />
                  </div>
                ))}
              </div>
            ) : playlists.length === 0 ? (
              ...
              <div className="text-center py-12 sm:py-16 px-4">
                <div className="inline-block mb-4 sm:mb-6 relative">
                  <div className="w-16 h-16 sm:w-24 sm:h-24 bg-gray-100 rounded-xl sm:rounded-2xl flex items-center justify-center mx-auto">
                    <Headphones className="w-8 h-8 sm:w-12 sm:h-12 text-gray-400" />
                  </div>
              ...
              // Lista de playlists - Layout mobile otimizado
              <div className="space-y-3 sm:space-y-4">
                {playlists.map((playlist, index) => (
                  <div
                    key={playlist.id}
                    className="group bg-white rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-gray-200 hover:border-purple-300 transition-all duration-300 hover:shadow-lg"
                  >
                    <div className="flex gap-3 sm:gap-4 items-start">
                      {/* Capa da playlist */}
                      <div className="relative flex-shrink-0">
                        <div className="w-14 h-14 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-lg sm:rounded-xl overflow-hidden shadow-sm group-hover:shadow-md transition-all duration-300">
                          <img
                      ...
                      {/* Informações - Layout vertical no mobile */}
                      <div className="flex-1 min-w-0">
                        <h3 className="text-sm sm:text-lg font-bold text-gray-900 mb-1 group-hover:text-purple-600 transition-colors duration-300 line-clamp-2 sm:line-clamp-1">
                          {playlist.nome}
                        </h3>
                        ...
                        {/* Informações secundárias - empilhadas no mobile */}
                        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 text-xs sm:text-sm text-gray-600">
                          ...
                          {/* Badge Spotify - sempre visível */}
                          <span className="inline-block px-1.5 py-0.5 bg-purple-100 text-purple-700 text-xs rounded-md font-medium border border-purple-200 w-fit mt-1 sm:mt-0">
                            Spotify
                          </span>
                        </div>
                      </div>

                      {/* Botão de abrir - ajustado para mobile */}
                      <div className="flex items-center flex-shrink-0">
                        <a
                          href={playlist.url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="bg-purple-600 hover:bg-purple-700 text-white px-3 py-2 sm:px-4 sm:py-3 rounded-lg sm:rounded-xl font-semibold transition-all duration-300 flex items-center gap-1.5 sm:gap-2 shadow-sm hover:shadow-md active:scale-95"
                        >
                          <span className="text-xs sm:text-sm">Ouvir</span>
