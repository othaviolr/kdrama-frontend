...
export interface DoramaReviewsRef {
  recarregar: () => void;
}

const DoramaReviews = forwardRef<DoramaReviewsRef, DoramaReviewsProps>(
  ({ doramaId, titulo }, ref) => {
    ...
    const carregarAvaliacoes = async (pagina: number = 1) => {
      setLoading(true);
      try {
        console.log('Carregando avaliações do dorama:', doramaId);
        const response = await avaliacaoService.getAvaliacoesDorama(doramaId);

        const avaliacoesFormatadas: AvaliacaoPublica[] = response.map(
          (avaliacao) => ({
            id: avaliacao.id,
            usuarioNome: avaliacao.usuarioNome,
            usuarioFoto: avaliacao.usuarioFoto,
            nota: avaliacao.nota,
        ...
        console.log('Avaliações carregadas:', avaliacoesFormatadas.length);
      } catch (error) {
    ...
    useImperativeHandle(ref, () => ({
      recarregar: () => carregarAvaliacoes(1),
    }));
...
DoramaReviews.displayName = 'DoramaReviews';

export default DoramaReviews;
