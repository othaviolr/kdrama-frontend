        ...
        <div className="relative bg-gradient-to-br from-white via-purple-50/30 to-white rounded-3xl p-6 shadow-xl border border-purple-100/50 overflow-hidden">
          ...
          <div className="space-y-3">
            <button
              ...
              className="group w-full bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-500 hover:to-purple-600 text-white py-3 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center gap-2 shadow-md hover:shadow-purple-500/20 transform hover:scale-[1.01]"
            >
              <Plus className="w-4 h-4 group-hover:rotate-90 transition-transform duration-300" />
              Adicionar à Lista
            </button>

            {/* Botão de Progresso Atualizado */}
            <button
              onClick={openModal}
              className={`group w-full border py-3 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center gap-2 shadow-sm hover:shadow-sm transform hover:scale-[1.01] ${progressoInfo.colorClass}`}
            >
              <progressoInfo.icon className="w-4 h-4 group-hover:scale-110 transition-transform duration-300" />
              {progressoInfo.label}
              {progressoInfo.hasProgress && (
                <span className="text-xs bg-white/80 px-2 py-1 rounded-full ml-1">
                  {progressoInfo.episodiosAssistidos}/
                  {dorama.temporadas[0]?.numeroEpisodios || 0}
                </span>
              )}
            </button>
